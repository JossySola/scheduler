    ...
    return (
        <section className="p-10 pb-[6rem] text-base/7 h-fit">
            ...
            <footer className="text-tiny dark:bg-[#1A1A1A]" id="footer">
                ...
                <p><sup>2</sup> <Link href="https://en.wikipedia.org/wiki/Argon2">Argon2id</Link> is a hybrid version of <b>Argon2</b>. We hash all passwords using Argon2id, a secure password hashing algorithm recommended by security experts.</p>
                <p><sup>3</sup> <Link href="https://docs.aws.amazon.com/kms/latest/developerguide/overview.html">AWS KMS</Link> is the <b>Key Management Service</b> provided by the <b>Amazon Web Services</b>. We use AWS’s secure key management infrastructure to encrypt sensitive data.</p>
                <p><sup>4</sup> Sensitive data is encrypted before being stored in our database using a strong encryption algorithm and securely managed keys.</p>
                <p><sup>5</sup> <Link href="https://www.anthropic.com/legal/privacy">Anthropic Privacy Policy.</Link> For AI-related features, we use a privacy-compliant language model provided by Anthropic.</p>
                <p><sup>6</sup> <Link href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Cookies">Cookies</Link> are small pieces of data a Server sends to the User's browser in order to preserve and manage information between the Client and the Server. In this case, we use Cookies to preserve and manage the User's authentication and session while using the application if they sign in.</p>
            </footer>
        </section>
    )
}