...
export default function ContactForm ({ lang }: {
    ...
    return <Form className="w-full p-5 sm:w-[400px] flex flex-col items-center">
        <Select label={ lang === "es" ? "¿Cuál es el asunto? " : "What's the subject? " } name="subject" isRequired>
            <SelectItem key="bug issue">{ lang === "es" ? "Problemas técnicos" : "Bug / Error Issue" }</SelectItem>
            <SelectItem key="account inquiry">{ lang === "es" ? "Consulta sobre la cuenta" : "Account Inquiry" }</SelectItem>
            <SelectItem key="general inquiry">{ lang === "es" ? "Consulta General" : "General Inquiry" }</SelectItem>
        </Select>
        <Input isRequired isClearable type="name" name="name" label={ lang === "es" ? "Nombre " : "Name " } />
        <Input isRequired isClearable type="email" name="email" label={ lang === "es" ? "Correo electrónico " : "E-mail " } />
        ...
        <Button type="submit" isLoading={ contactPending } isDisabled={ contactPending } className="bg-black text-white" formAction={ formAction }>
            { lang === "es" ? "Enviar" : "Send" }
        </Button>
    </Form>
}