...
export default function ColumnSettings () {
    ...
    return (
        <section className="w-full p-5 flex flex-col justify-start items-center magicBorder">
            ...
            <Tabs aria-label="column-settings">
                {
                    columnHeaders && columnHeaders.map((column, columnIndex) => {
                        if (columnIndex !== 0) {
                            return (
                                <Tab key={ `tab-${columnIndex}` } title={ column ? column : lang === "es" ? "Sin nombre" : "No name" }>
                                    <ColumnTabCard columnIndex={ columnIndex } name={ column } />
                                </Tab>
                            )
                        }
                    })
