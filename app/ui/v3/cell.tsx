...
export default function Cell ({ element, rowIndex, colIndex }: { 
    ...
    if (rowIndex === 0 && colIndex !== 0) {
        if (table.columnType === "date") {
            const verification = zod.iso.date().safeParse(element.value);
            return <DatePicker
            id={label}
            name={label}
            size="lg"
            ...
            defaultValue={ verification.success ? parseDate(element.value) : null }
            onChange={e => {
                debouncedModification((e && e.toString()) ?? "");
                if (colIndex === 1 && e) {
                    let currentDate = new CalendarDate(e.year, e.month, e.day);
                    let newDate = currentDate;
                    if (table.rows[0] && table.rows[0].size > 0) {
                        table.rows[0].forEach((header, key) => {
                            if (key !== "A0") {
                                header.value = newDate.toString();
                                currentDate = newDate;
        ...
        if (table.columnType === "time") {
            const verification = zod.iso.time().safeParse(element.value);
            return <TimeInput
            id={label}
            name={label}
            size="lg"
            ...
            defaultValue={ verification.success ? parseTime(element.value) : null }
            onChange={e => {
                debouncedModification((e && e.toString()) ?? "");
                if (colIndex === 1 && e) {
